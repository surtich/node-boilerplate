...
function OperationalErrorDecider () {
  this.isOperationalError = function (error) {
    for (let rule of this.getAllRules()) {
      if (rule(error)) {
        logger.info(
          `While deciding whether an error is operational, this error found to be operational: ${
            error.name
          }: ${error.message}`
        )
        return true
  ...
  this.getAllRules = function () {
    ...
    allRules.push(error => {
      return (
        error.hasOwnProperty('table') &&
        error.hasOwnProperty('internalQuery') &&
        error.hasOwnProperty('position')
      )
    })
